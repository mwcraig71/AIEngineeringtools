/**
 * Application controller for Composite Steel Girder Load Rating.
 */
...
(function () {
  ...
  tabs.forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      if (tab.dataset.tab === 'deck') updateDeckDisplay();
    });
  });
  ...
  // ---- Deck property display ----
  function updateDeckDisplay() {
    const fc = parseFloat(document.getElementById('concreteFc').value) || 4.0;
    const ts = parseFloat(document.getElementById('slabThickness').value) || 8.0;
    const spanFt = parseFloat(document.getElementById('spanLength').value) || 60;
    const girderSpacing = parseFloat(document.getElementById('girderSpacing').value) || 96;
    const beffOverride = parseFloat(document.getElementById('beffOverride').value) || 0;

    const { n, Ec } = computeModularRatio(fc);
    document.getElementById('nDisplay').textContent = n;
    document.getElementById('ecDisplay').textContent = Ec.toFixed(0) + ' ksi';
    ...
    if (beffOverride > 0) {
      ...
      beff = computeEffectiveWidth(spanFt, ts, girderSpacing, tw);
    }
    document.getElementById('beffDisplay').textContent = beff.toFixed(1) + ' in';
  }
  updateDeckDisplay();

  // Update deck display when relevant inputs change
  ['concreteFc', 'slabThickness', 'spanLength', 'girderSpacing', 'beffOverride'].forEach(id => {
    document.getElementById(id).addEventListener('change', updateDeckDisplay);
  });
  ...
  function getSteelArea() {
    ...
    return bfc * tfc + D * tw + bft * tft;
  }

  function updateDeadLoadDisplay() {
    const steelArea = getSteelArea();
    const selfWt = steelArea * 490 / 12000; // kip/ft
    document.getElementById('dcSteelSelf').textContent = selfWt.toFixed(4) + ' kip/ft';

    // Deck weight = beff * ts * 150 pcf / 144 / 1000 * 12 = beff * ts * 150 / 12000 kip/ft
    // Actually: weight per ft of girder = (girder spacing * ts * 150) / 144 / 1000 * 12
    // Simpler: w = spacing_in * ts_in * 150 pcf / (144 in2/ft2) / 1000 lb/kip
    const ts = parseFloat(document.getElementById('slabThickness').value) || 8.0;
    const girderSpacing = parseFloat(document.getElementById('girderSpacing').value) || 96;
    const deckWt = girderSpacing * ts * 150 / 144 / 1000; // kip/ft
    document.getElementById('dcDeckWeight').textContent = deckWt.toFixed(4) + ' kip/ft';

    // Haunch weight
    const haunch = parseFloat(document.getElementById('haunchDepth').value) || 0;
    const isRolled = document.querySelector('input[name="sectionType"]:checked').value === 'rolled';
    let bfc;
    if (isRolled) {
      const ws = getWShapeProps(wShapeSelect.value);
    ...
    const haunchWt = bfc * haunch * 150 / 144 / 1000; // kip/ft
    document.getElementById('dcHaunchWeight').textContent = haunchWt.toFixed(4) + ' kip/ft';

    const dc1Other = parseFloat(document.getElementById('dc1Other').value) || 0;
    const dc1Total = selfWt + deckWt + haunchWt + dc1Other;
    document.getElementById('dc1Total').textContent = dc1Total.toFixed(4) + ' kip/ft';
  }
  updateDeadLoadDisplay();
  ...
  function getInputs() {
    ...
    const deck = { ts, fc, haunch, girderSpacing, beffOverride };
    ...
    // Shear connectors
    const studs = {
      diameter: parseFloat(document.getElementById('studDiameter').value) || 0.75,
      Fu: parseFloat(document.getElementById('studFu').value) || 60,
      perRow: parseInt(document.getElementById('studsPerRow').value) || 2
    };

    // Dead loads
    const steelArea = getSteelArea();
    const selfWt = steelArea * 490 / 12000;
    const deckWt = girderSpacing * ts * 150 / 144 / 1000;

    const isRolled = sectionType === 'rolled';
    let bfc;
    if (isRolled) {
    ...
    const haunchWt = bfc * haunch * 150 / 144 / 1000;
    const dc1Other = parseFloat(document.getElementById('dc1Other').value) || 0;
    const dc1W = selfWt + deckWt + haunchWt + dc1Other;
    const dc2W = parseFloat(document.getElementById('dc2Load').value) || 0;
  ...
  window.runRating = function () {
    ...
    try {
      const result = runCompositeRating(inputs);
  ...
  function displayResults(r, inputs) {
    ...
    summaryBox.innerHTML = `
      ...
      <div class="summary-details">
        ...
        <strong>Section:</strong> ${sectionLabel} (Composite) | <strong>F<sub>y</sub>:</strong> ${inputs.Fy} ksi | <strong>Span:</strong> ${inputs.spanFt} ft<br>
        ...
        <strong>Composite:</strong> n = ${r.n} | b<sub>eff</sub> = ${r.beff.toFixed(1)} in | t<sub>s</sub> = ${inputs.deck.ts} in
    ...
    const bs = r.steelSection;
    html += `<div class="result-section">
      <h3>Steel Section Properties (${sectionLabel})</h3>
      <table class="result-table">
        ...
        <tr><td>I<sub>x</sub> (steel)</td><td>${bs.Ix.toFixed(0)} in&sup4;</td><td>S<sub>xc</sub> (steel top)</td><td>${bs.Sxc.toFixed(1)} in&sup3;</td></tr>
        <tr><td>S<sub>xt</sub> (steel bot)</td><td>${bs.Sxt_tens.toFixed(1)} in&sup3;</td><td>Z<sub>x</sub> (steel)</td><td>${bs.Zx.toFixed(1)} in&sup3;</td></tr>
      </table>
    </div>`;

    // Composite section properties
    const cst = r.compositeST;
    const clt = r.compositeLT;
    html += `<div class="result-section">
      <h3>Composite Section Properties</h3>
      <table class="result-table">
        <tr><th colspan="2">Short-Term (n = ${r.n})</th><th colspan="2">Long-Term (3n = ${3 * r.n})</th></tr>
        <tr><td>I<sub>comp</sub></td><td>${cst.Icomp.toFixed(0)} in&sup4;</td><td>I<sub>comp</sub></td><td>${clt.Icomp.toFixed(0)} in&sup4;</td></tr>
        <tr><td>S<sub>bot</sub> (steel)</td><td>${cst.SbotSteel.toFixed(1)} in&sup3;</td><td>S<sub>bot</sub> (steel)</td><td>${clt.SbotSteel.toFixed(1)} in&sup3;</td></tr>
        <tr><td>S<sub>top,slab</sub></td><td>${cst.StopSlab.toFixed(1)} in&sup3;</td><td>S<sub>top,slab</sub></td><td>${clt.StopSlab.toFixed(1)} in&sup3;</td></tr>
        <tr><td>NA from steel top</td><td>${cst.ybarComp.toFixed(2)} in</td><td>NA from steel top</td><td>${clt.ybarComp.toFixed(2)} in</td></tr>
        <tr><td>b<sub>eff</sub></td><td colspan="3">${r.beff.toFixed(1)} in (E<sub>c</sub> = ${r.Ec.toFixed(0)} ksi)</td></tr>
      </table>
    </div>`;

    // Base flexural capacity (from first non-loss point, or first point)
    const basePt = r.pointResults.find(p => !p.hasLoss) || r.pointResults[0];
    const m = basePt.moment;
    html += `<div class="result-section">
      <h3>Base Flexural Capacity (Positive Moment)</h3>
      <table class="result-table">
        <tr><td>M<sub>n</sub></td><td>${m.Mn.toFixed(1)} kip-ft</td><td>M<sub>p</sub> (composite)</td><td>${m.Mp.toFixed(1)} kip-ft</td></tr>
        <tr><td>M<sub>y</sub> (composite)</td><td>${m.My.toFixed(1)} kip-ft</td><td>Governs</td><td>${m.governs}</td></tr>
        <tr><td>Web class</td><td>${m.webClass}</td><td>Flange class</td><td>${m.flangeClass}</td></tr>
        <tr><td>D<sub>p</sub></td><td>${m.Dp.toFixed(2)} in</td><td>D<sub>t</sub></td><td>${m.Dt.toFixed(2)} in</td></tr>
        <tr><td>D<sub>p</sub>/D<sub>t</sub></td><td>${m.DpDt.toFixed(4)}</td><td>Ductility</td><td>${m.ductilityOK ? 'OK (&le; 0.42)' : 'FAILS (&gt; 0.42)'}</td></tr>
        <tr><td>&phi;M<sub>n</sub></td><td colspan="3"><strong>${basePt.phiMn.toFixed(1)} kip-ft</strong></td></tr>
      </table>
    </div>`;

    // Base shear capacity
    const sh = basePt.shear;
    html += `<div class="result-section">
      <h3>Base Shear Capacity</h3>
      <table class="result-table">
        <tr><td>V<sub>n</sub></td><td>${sh.Vn.toFixed(1)} kip</td><td>&phi;V<sub>n</sub></td><td><strong>${basePt.phiVn.toFixed(1)} kip</strong></td></tr>
        <tr><td>V<sub>p</sub></td><td>${sh.Vp.toFixed(1)} kip</td><td>C</td><td>${sh.C.toFixed(3)}</td></tr>
        <tr><td>D/t<sub>w</sub></td><td>${sh.webRatio.toFixed(1)}</td><td>k</td><td>${sh.k.toFixed(2)}</td></tr>
      </table>
    </div>`;

    // Shear connectors
    if (r.shearConnectors) {
      const sc = r.shearConnectors;
      html += `<div class="result-section">
        <h3>Shear Connector Design</h3>
        <table class="result-table">
          <tr><td>Q<sub>n</sub> (per stud)</td><td>${sc.Qn.toFixed(2)} kip</td><td>A<sub>sc</sub></td><td>${sc.Asc.toFixed(4)} in&sup2;</td></tr>
          <tr><td>V<sub>h</sub> (interface shear)</td><td>${sc.Vh.toFixed(1)} kip</td><td>Controls</td><td>${sc.Vh === sc.Vh1 ? 'Concrete' : 'Steel'}</td></tr>
          <tr><td>0.85f\'<sub>c</sub>b<sub>eff</sub>t<sub>s</sub></td><td>${sc.Vh1.toFixed(1)} kip</td><td>A<sub>s</sub>F<sub>y</sub></td><td>${sc.Vh2.toFixed(1)} kip</td></tr>
          <tr><td>Studs required (half-span)</td><td>${sc.nRequired}</td><td>Rows (${sc.studsPerRow}/row)</td><td>${sc.nRows}</td></tr>
          <tr><td>Max pitch</td><td colspan="3">${sc.pitch.toFixed(1)} in</td></tr>
        </table>
      </div>`;
    }

    // Rating results per check point
  ...
  function renderPointResult(pt, inputs) {
    ...
    if (pt.hasLoss && pt.section) {
      const s = pt.section;
      html += `<table class="result-table">
        <tr><th colspan="4">Reduced Steel Section at This Location</th></tr>
    ...
    html += `<table class="result-table" style="margin-top:0.5rem">
      ...
      <tr><td>DC1 Moment</td><td>${pt.deadLoads.dc1Moment.toFixed(1)} kip-ft</td><td>DC1 Shear</td><td>${pt.deadLoads.dc1Shear.toFixed(1)} kip</td></tr>
      <tr><td>DC2 Moment</td><td>${pt.deadLoads.dc2Moment.toFixed(1)} kip-ft</td><td>DC2 Shear</td><td>${pt.deadLoads.dc2Shear.toFixed(1)} kip</td></tr>
